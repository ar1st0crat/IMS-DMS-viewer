...
# ----------------- fix a dims: move the clipped region up to the right place
def fix( signal ):
    for i in range( 1, len(signal) ):
        if signal[i] == -32768 and signal[i-1] > 0:
            signal[i] = 32767
            
# ------------------------------------ alternative method for fixing the dims
def fix_alternative( signal ):
    clips = []
    for i in range(1,len(signal)):
        if abs(signal[i]) - abs(signal[i-1]) < 100 and \
           abs(signal[i]) > upperClipThreshold:
            clips.append(i)
...
def findSpikes(signal, deriv):
    ...
    for i in range(2,len(signal)-2):
        if (abs(signal[i] - signal[i-2]) > lowerClipThreshold/3) and \
           (abs(signal[i] - signal[i+2]) > lowerClipThreshold/3):
...
def findClips(signal, deriv, maxValue = 32767.0):
    ...
    for i in range(len(deriv)):
        if abs(deriv[i]) < 5 and abs(signal[i]) > upperClipThreshold:
            clips.append(i)
    return clips
...
def filterHills(signal, hills):
    for i in range(1,len(hills)-1):
        if abs(signal[hills[i]]) < lowerClipThreshold: #4000:
            if abs(signal[hills[i-1]]) < lowerClipThreshold and \
               abs(signal[hills[i+1]]) < lowerClipThreshold:
                hills[i] = 0
...
dims = np.array([int(fi) for fi in full_info[1:-1]])
...
# uncomment this if ou want to compare your peaks with peaks found via cwt
